...
func (t *TestBuffer)Peek()(byte,error){
	if t.off==t.len{
		return 0, Timer.ExecError{Info:"EOF"}
	}
...
func (t* TestBuffer)Write(p []byte)(n int, err error){
	...
	if t.len+plen>t.siz{
		t.out = true
		return 0,Timer.ExecError{Info:"OLE"}
	}
	tmp:=t.buf[t.off:]
	copy(tmp,p)
	t.len+=plen
...
func CompileBase(cmd string ,timeLimit ,memoryLimit int64)TestInfo{
	stderr :=bytes.Buffer{}
	timeUse,memoryUse,err := Timer.LimitExecE(timeLimit,memoryLimit,cmd,&stderr)
	var res TestInfo
	res.Statu="006"
	if err != nil{
		if err.Error()=="MLE"{
			res.Info="编译使用内存超限"
		}else if err.Error()=="exit status 137"{
			res.Info="编译超时"
		}else {
			res.Info=err.Error()
		}
...
func RunBase(cmd ,in,out string,timeLimit,memoryLimit int64)TestInfo{
	...
	timeUse,memoryUse,err:=Timer.LimitExec(timeLimit,memoryLimit,cmd,infile,&stdout,&stderr)
	res:=TestInfo{}
	if err!=nil{
		if err.Error()=="MLE"{
			res.Info="内存超限"
